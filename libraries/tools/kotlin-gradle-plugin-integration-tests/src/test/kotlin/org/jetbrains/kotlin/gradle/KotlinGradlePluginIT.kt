...
class KotlinGradleIT : BaseGradleIT() {
    ...
    fun testBuildMetricsSmokeTest() = with(Project("simpleProject")) {
        ...
        assertTrue { report.contains("LOOKUP_SIZE:") }
    ...
    fun testCompilerBuildMetricsSmokeTest() {
        val unmodifiableMapClass = Class.forName(Collections::class.java.name + "\$UnmodifiableMap")
        val privateCollectionField = unmodifiableMapClass.getDeclaredField("m");
        privateCollectionField.isAccessible = true
        @Suppress("UNCHECKED_CAST")
        val privateCollection = privateCollectionField.get(System.getenv()) as MutableMap<String, String?>
        privateCollection["KOTLIN_REPORT_PERF"] = "true"
